        ...
        else
        {
            ...
            while(1)
            {
                ...
                if(my_isnumeric(Param[1]))
                {
                    ...
                    if(x < 0)
                    {
                        ErrorMsg = "Sub array index below 0";
                        NEXT_STATE(PARSE_ERROR);
                    }
...
int cmdParseHandleMath(char **Param, int ParamCount, FunctionStruct *Func, RobotStruct *Robot)
{
    ...
    //how to handle sin() and atan2(x,y)
    //probably see () and matches MathOrder1 so call handler
    DidMathParse = 0;
    while(1)
    {
        InnerMost = -1;
...
int cmdParseName(char **Params[], FunctionStruct *Func, RobotStruct *Robot)
{
    ...
    //avoid the name smash bug
    if(Len >= sizeof(Robot->Name))
        Len = sizeof(Robot->Name) - 1;

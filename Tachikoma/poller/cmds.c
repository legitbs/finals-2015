...
MyDouble GetVariable(RobotStruct *RobotData, FunctionStruct *Func, GlobalVarStruct *GlobalEntry)
{
    ...
    switch((unsigned char)Func->Cmds[Func->CmdPos])
    {
        ...
        case PARAM_TEMP:
            //temporary variable
            if((unsigned char)Func->Cmds[Func->CmdPos+1] > 9)
            {
                ErrorMsg = "Temp variable invalid";
        ...
        case PARAM_SUBARRAY_NUM:
            //BUG: we don't validate negative indexing
            if((*(int *)&Func->Cmds[Func->CmdPos+1] < 0) || (*(int *)&Func->Cmds[Func->CmdPos+1] >= GlobalEntry->ArraySize))
            {
                ErrorMsg = "Subarray index out of range";
                NEXT_STATE(CMD_ERROR);
            }
        ...
        case PARAM_SUBPROP_NUM:
            if((*(unsigned char *)&Func->Cmds[Func->CmdPos+1] < 0) || (*(unsigned char *)&Func->Cmds[Func->CmdPos+1] >= GlobalEntry->PropSize))
            {
                ErrorMsg = "Property index out of range";
                NEXT_STATE(CMD_ERROR);
            }

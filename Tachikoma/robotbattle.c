...
{
    //before people bitch, i know this is a massive function, it was done on purpose due to the state entries
    //instead of breaking it up into many sub functions
    volatile int Initialized = 0;
...
int main(int argc, char **argv)
{
    ...
    //brute forceable if you think about it
    srand((int)main);
    size = rand();
    srand(size);
    my_printf("Seed: %d\n", size);
    ...
    if(CurRobotID == (RobotCount - 1))
        fd = open("/home/tachikoma/server_robot.prg", O_RDONLY);
